...
void CTestView::ReadVertex()//点表
{
	...
	for(int i=0;i<N1-1;i++)
	{
		...
		for(int j=0;j<N2;j++)
	    {
			...
			T[i*N2+j+1].u=gBeta1/(2*PI)*(nWidth-1);
			T[i*N2+j+1].v=(PI-gAlpha1)/PI*(nHeight-1);
		}
	}
	//计算南极点坐标
		...
		{			
			...
			if(Dot(ViewVector,F[i][j].fNormal)>=0)
			{
				if(3==F[i][j].vN)//处理三角形面片
				{
					...
					Texture3[0].u=gBeta*(j)/360.0*(nWidth-1);Texture3[0].v=(1.0-gAlpha*i/180.0)*(nHeight-1);  
					Texture3[1].u=gBeta*(j)/360.0*(nWidth-1);Texture3[1].v=(1.0-gAlpha*tempi/180.0)*(nHeight-1);
					Texture3[2].u=gBeta*tempj/360.0*(nWidth-1);Texture3[2].v=(1.0-gAlpha*i/180.0)*(nHeight-1);
					zbuf->SetPoint(Point3,Normal3,Texture3,3);//初始化
					zbuf->CreateBucket();//创建桶表
					zbuf->CreateEdge();//创建边表
					zbuf->Phong(pDC,ViewPoint,pLight,pMaterial,LImage1);//填充三角形
					zbuf->ClearMemory();
				}
				else//处理四边形面片
				{
					...
					if(N2-1==j)//消除图像纹理的接缝
					{
						Texture4[2].u=nWidth-1;
						Texture4[3].u=nWidth-1;
					}
					zbuf->SetPoint(Point4,Normal4,Texture4,4);//初始化
					zbuf->CreateBucket();//创建桶表
					zbuf->CreateEdge();//创建边表
					zbuf->Phong(pDC,ViewPoint,pLight,pMaterial,LImage1);//填充四边形
					zbuf->ClearMemory();
				}				
			}
		}
...
void CTestView::ReadImage()//读入颜色纹理
{
    CBitmap NewBitmap;
	NewBitmap.LoadBitmap(IDB_CLRTEXTURE);//导入DDB位图
	NewBitmap.GetBitmap(&bmp1);//将CBitmap的信息保存到Bitmap结构体中
	int nbytesize=bmp1.bmWidthBytes*bmp1.bmHeight;
	im=new BYTE[nbytesize];
	NewBitmap.GetBitmapBits(nbytesize,(LPVOID)im);
	Image1=new COLORREF*[bmp1.bmHeight];
	for(int n1=0;n1<bmp1.bmHeight;n1++)
		Image1[n1]=new COLORREF[bmp1.bmWidth];
	for(n1=0;n1<bmp1.bmHeight;n1++)
	{
		for(int n2=0;n2<bmp1.bmWidth;n2++)
		{
			int pos=n1*bmp1.bmWidthBytes+4*n2;//颜色分量位置
			n1=bmp1.bmHeight-1-n1;//位图从左下角向右上角绘制
			Image1[n1][n2]=RGB(im[pos+2],im[pos+1],im[pos]);
		}
	}
	delete []im;
	LImage1=new COLORREF*[nHeight];
	for(n1=0;n1<nHeight;n1++)
		LImage1[n1]=new COLORREF[nWidth];
	BiLinear_BMP(LImage1,nWidth,nHeight,Image1,bmp1.bmWidth,bmp1.bmHeight);
}
...
void CTestView::ReadBumpMap()//读入凹凸纹理
{
	...
	NewBitmap.GetBitmap(&bmp2);//将CBitmap的信息保存到Bitmap结构体中
	int nbytesize=bmp2.bmWidthBytes*bmp2.bmHeight;
	im=new BYTE[nbytesize];
	NewBitmap.GetBitmapBits(nbytesize,(LPVOID)im);
	Image2=new COLORREF*[bmp2.bmHeight];
	for(int n1=0;n1<bmp2.bmHeight;n1++)
		Image2[n1]=new COLORREF[bmp2.bmWidth];
	for(n1=0;n1<bmp2.bmHeight;n1++)
	{
		for(int n2=0;n2<bmp2.bmWidth;n2++)
		{
			int pos=n1*bmp2.bmWidthBytes+4*n2;//颜色分量位置
			n1=bmp2.bmHeight-1-n1;//位图从左下角向右上角绘制
			Image2[n1][n2]=RGB(im[pos+2],im[pos+1],im[pos]);
		}
	}
	LImage2=new COLORREF*[nHeight];
	for(n1=0;n1<nHeight;n1++)
		LImage2[n1]=new COLORREF[nWidth];
	BiLinear_BMP(LImage2,nWidth,nHeight,Image2,bmp2.bmWidth,bmp2.bmHeight);
	Imgx_Gradient=new double*[nHeight];
	Imgy_Gradient=new double*[nHeight];
	for(n1=0;n1<nHeight;n1++)
	{
		Imgx_Gradient[n1]=new double[nWidth];
		Imgy_Gradient[n1]=new double[nWidth];
	}
	for(n1=0;n1<nHeight;n1++)
	{
		for(int n2=0;n2<nWidth;n2++)
		{
			...
			if(fonty>nWidth-1)
				fonty=nWidth-1;
			Imgx_Gradient[n1][n2]=-(GetRValue(LImage2[n1][fonty])-GetRValue(LImage2[n1][backy]));
			Imgy_Gradient[n1][n2]=-(GetRValue(LImage2[fontx][n2])-GetRValue(LImage2[backx][n2]));
		}
	}
	delete []im;
}
