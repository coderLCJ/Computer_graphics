...
void CZBuffer::SetPoint(CPi3 *p,CVector *n,CP3 *t,int m)
{
	P=new CPi3[m];
	N=new CVector[m];
	T=new CP3[m];
...
void CZBuffer::Phong(CDC *pDC,CP3 ViewPoint,CLighting *pLight,CMaterial *pMaterial)//填充多边形
{
	...
	for(pCurrentB=pHeadB;pCurrentB!=NULL;pCurrentB=pCurrentB->pNext)
	{
		...
		CP3 ta,tb,tf;//ta和tb代表边上任意点的三维纹理，tf代表面上任意点的三维纹理
		...
		for(pT1=pHeadE;pT1!=NULL;pT1=pT1->pNext)
		{
			...
			else
			{
				...
				for(double x=xb;x<xe;x++)//左闭右开
				{
					...
					CRGB Textureclr=ReadWoodTexture(tf);
...
CP3 CZBuffer::Interpolation(double t,double t1,double t2,CP3 tex1,CP3 tex2)//纹理线性插值
{
	CP3 texture;
