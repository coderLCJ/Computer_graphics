...
CTestView::CTestView()
{
    ...
    pLight->Light[0].SetPosition(800,0,0);//设置光源位置坐标
...
void CTestView::DrawObject(CDC* pDC)//绘制长方体
{
	CPi3 Point[4];//面的顶点坐标
	CP3  Texture[4];//面的纹理坐标
	CVector Normal4[4];
	CZBuffer *zbuf=new CZBuffer;
	zbuf->InitDeepBuffer(1000,1000,1000);//初始化深度缓冲器
	...
	{	
		...
		if(Dot(ViewVector,F[nFace].fNormal)>=0)//背面剔除
		{
			...
			zbuf->SetPoint(Point,Normal4,Texture,4);//初始化（绑定顶点和各个顶点的纹理坐标点）
