...
#include "ZBuffer.h"
#include "math.h"
#define Round(f) int(f+0.5)//四舍五入宏定义
#ifdef _DEBUG
#undef THIS_FILE
...
void CZBuffer::SetPoint(CPi3 *p,CVector *n,int m)
{
	P=new CPi3[m];
	N=new CVector[m];
    for(int i=0;i<m;i++)
...
void CZBuffer::Phong(CDC *pDC,CP3 ViewPoint,CLighting *pLight,CMaterial *pMaterial,int Sign)//填充多边形
{
		...
		for(pT1=pHeadE;pT1!=NULL;pT1=pT1->pNext)
		{
			...
			else
			{
				...
				for(double x=xb;x<xe;x++)//左闭右开
				{
					...
					if(z<=zBuffer[Round(x)+Width/2][pCurrentB->ScanLine+Height/2])//如果新采样点的深度大于原采样点的深度
					{
						if(SPHERE==Sign)//球不透明，写入帧缓冲
						{
							cBuffer[Round(x)+Width/2][pCurrentB->ScanLine+Height/2]=c;
							zBuffer[Round(x)+Width/2][pCurrentB->ScanLine+Height/2]=z;
							pDC->SetPixelV(Round(x),pCurrentB->ScanLine,RGB(c.red*255,c.green*255,c.blue*255));
						}
						else
						{
							CRGB ctrsns=GetTransColor(c,cBuffer[Round(x)+Width/2][pCurrentB->ScanLine+Height/2]);
							zBuffer[Round(x)+Width/2][pCurrentB->ScanLine+Height/2]=z;
							pDC->SetPixelV(Round(x),pCurrentB->ScanLine,RGB(ctrsns.red*255,ctrsns.green*255,ctrsns.blue*255));
						}
					}
					z+=zStep;
				}
...
CRGB CZBuffer::GetTransColor(CRGB clr1,CRGB clr2)//计算透明颜色
{
	CRGB color;
	double t=P[0].c.alpha;//透明度
	color=(1-t)*clr1+t*clr2;
	return color;
}

void CZBuffer::InitDeepBuffer(int Width,int Height,double Depth,CRGB BkClr)//初始化深度与颜色缓存
{
	this->Width=Width,this->Height=Height;
	zBuffer=new double *[Width];//深度缓存器
	cBuffer=new CRGB *[Width];//颜色缓存器
	for(int i=0;i<Width;i++)
	{
		zBuffer[i]=new double[Height];
		cBuffer[i]=new CRGB[Height];
	}
	for(i=0;i<Width;i++)//初始化深度缓存
