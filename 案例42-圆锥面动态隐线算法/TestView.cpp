...
void CTestView::ReadVertex()//点表
{
	...
	V[0].x=0;V[0].y=0;V[0].z=0;//圆锥底面中心
    for(int i=0;i<N2;i++)//纵向
	{	
		cNum1=i*cNum;
		for(int j=0;j<N1;j++)//周向
		{
		    ...
		    double r1=(N2-i)*r/N2;
			V[i*N1+j+1].x=r1*cos(cTheta1);
			V[i*N1+j+1].y=i*cNum;	
			V[i*N1+j+1].z=r1*sin(cTheta1);
		}
	}
    V[N1*N2+1].x=0;V[N1*N2+1].y=h;V[N1*N2+1].z=0;//圆锥顶点
}

void CTestView::ReadFace()//面表
{
	//设置二维动态数组
	F=new CFace *[N2+1];//设置行
	for(int n=0;n<N2+1;n++)
	{
		F[n]=new CFace[N1];//设置列
	}
	for(int j=0;j<N1;j++)//构造底部三角形面片
	{
		...
		F[0][j].SetNum(3);
		for(int k=0;k<F[0][j].vN;k++)//传入面中点的索引
			F[0][j].vI[k]=BottomIndex[k];
	}
	for(int i=1;i<N2;i++)//构造圆锥侧面的四边形面片
	{
		for(int j=0;j<N1;j++)
	    {
			int tempi=i+1;
	...
	for(j=0;j<N1;j++)//构造侧面顶角附近的三角形面片
	{
		...
		F[N2][j].SetNum(3);	
		for(int k=0;k<F[N2][j].vN;k++)
			F[N2][j].vI[k]=TopIndex[k];
	}
}
...
void CTestView::DrawObject(CDC* pDC)//绘制圆锥网格
{
